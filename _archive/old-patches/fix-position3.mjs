...
// Find "title=\"Group\"" line to locate the hr before it
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('title="Group"')) {
    // Go backwards to find the hr line
    for (let j = i - 1; j >= i - 5; j--) {
      if (lines[j].includes('<hr')) {
        console.log('Found hr at line ' + (j+1) + ': ' + lines[j].trim());
        // Go back one more to find the button start
        for (let k = j - 1; k >= j - 3; k--) {
          if (lines[k].includes('<button onClick={async')) {
            console.log('Group button starts at line ' + (k+1));
            break;
          }
        }
        console.log('Will insert Position block before line ' + (j+1));
        ...
        lines.splice(j, 0, ...alignBlock);
        console.log('Done! Position block inserted.');
        break;
      }
    }
    break;
