...
### 임시 파일 금지
- 디버깅·테스트·일회성 스크립트는 반드시 `_scratch/` 폴더에만 생성
- `_scratch/`는 .gitignore에 포함되어 있음 — 절대 커밋되지 않음
- 루트 디렉토리에 `fix-*.js`, `check-*.js`, `diag-*.js`, `*.ps1`, `*-backup.txt`,
  `Untitled`, `$tmpPdf` 같은 파일을 만들지 말 것
- 같은 작업의 v2, v3, v4 파일 생성 금지 — 기존 파일을 수정하거나 git history로 추적

### 파일 크기 제한
- 단일 파일 800줄 초과 금지
- 800줄에 가까워지면 책임별로 분리할 시점이라고 먼저 제안할 것
- 현재 unified-editor.tsx는 2000줄+ → 다음 리팩토링 대상

### 타입 규율
- `any` 타입 사용 금지 — 위반 사항 발견 시 작업 끝낼 때 보고
- 동적 import는 `let _x: T | null = null` 패턴 사용

### 작업 종료 시 체크리스트
- [ ] `git status`에 의도하지 않은 파일이 없는가
- [ ] 임시 스크립트가 `_scratch/`가 아닌 곳에 있는가
- [ ] 새 파일이 800줄을 넘었는가
- [ ] `any` 타입을 새로 추가했는가
- [ ] CMYK 엔진처럼 silent fallback이 있다면 사용자 경고가 있는가
- 위 항목 중 하나라도 해당되면 사용자에게 보고

### CMYK 엔진 특수 규칙
- `cmyk-engine.ts`의 LUT 로딩이 실패하면 `console.error`만 하지 말고
  toast 알림 또는 UI 경고로 사용자에게 알릴 것
- 단순 수식 폴백(현재의 1-r/255 변환)을 silent로 사용하면 인쇄 색상이 틀어짐
- "FOGRA39 프로파일 로딩 실패 — 색상 정확도 보장 불가" 같은 명시적 메시지 필요
---
