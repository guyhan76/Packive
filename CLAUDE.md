...
### 자율로 진행 (멈추지 말 것 — 끝나고 한 번에 보고)
다음은 매 단계 승인 없이 끝까지 진행한 뒤 결과만 보고:
- UI/스타일/레이아웃 수정
- 기능 추가 (새 컴포넌트, 새 함수)
- 일반 버그 수정
- 텍스트/i18n/상수 변경
- 리팩토링 (단일 파일 내, 800줄 규칙 준수)
- 검증 명령 실행 (tsc, lint, build, test)
- 임시 디버그 스크립트 (_scratch/ 내)

이런 작업은 "진단→승인→실행"으로 쪼개지 말고, 계획을 짧게 한 줄로 말한 뒤 바로 실행하고, 끝나면 tsc 0 확인하고 한 번에 보고한다.

### 반드시 멈추고 승인받기 (위험 작업만)
- git 머지/rebase/force push/브랜치 삭제
- main 브랜치 직접 수정
- PDF/이미지 바이너리 조작
- DB 스키마/마이그레이션
- 200줄 이상 대규모 변경 또는 5개 이상 파일 동시 수정
- 의존성 추가/제거, 설정 파일(next.config, tsconfig) 변경
- 파일 대량 삭제/이동

### 보고 방식
- 자율 작업: 끝나고 "무엇을 했다 + tsc 결과 + 다음 제안" 3줄로 압축. 중간 과정 나열 금지.
- 장황한 표/단계별 중계 금지. 결과 중심으로.
- 질문은 한 번에 모아서. "이거 할까요? → 네 → 그럼 이거 할까요?" 식의 연쇄 금지.

### 커밋
- 자율 작업은 논리적 단위로 알아서 커밋. 매 커밋마다 승인 구하지 말 것.
- 단 위험 작업 커밋만 승인.
---
