...
{id:"ecma-b10-tray",m:"B10_02_00_00_Lid",L:200,W:150,D:40,Th:0.5,ng:1,ex:{Lid:"B10_02_00_00_A"}},
{id:"ecma-b20-hinged",m:"B20_01_00_00_Lid",L:200,W:150,D:40,Th:0.5,ng:1,ex:{Lid:"B20_02_00_00_A"}}
];
...
console.log("[GEN]",t.id,t.m);
for(let a=1;a<=3;a++){try{
const r=await fetch("http://localhost:3001/api/dieline",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(b)});
...
const s=await r.text();if(!s.includes("<svg")){console.log("  [FAIL] not svg");if(a<3)continue;return 0;}
fs.writeFileSync(f,s,"utf8");console.log("  [OK]",(fs.statSync(f).size/1024).toFixed(1)+"KB");return 1;
}catch(e){console.log("  [ERR]",a,e.message);if(a<3){await new Promise(r=>setTimeout(r,3000));continue;}return 0;}}return 0;}
async function main(){
if(!fs.existsSync(DIR))fs.mkdirSync(DIR,{recursive:true});
let ok=0,fail=0;
for(const t of T){const r=await gen(t);if(r)ok++;else fail++;await new Promise(r=>setTimeout(r,2000));}
