...
## 개발 스케줄 (2026-03-08 확정) - MUST READ EVERY SESSION

### Phase 1: 벡터 내보내기 + 텍스트 아웃라인 완성
**목표**: 모든 요소(도형, 텍스트, 표, 이미지)가 벡터로 내보내져 Illustrator에서 편집 가능하고 CTP 출력에 바로 사용 가능
**예상 기간**: 1일

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 1-1 | SVG 벡터 내보내기 테스트 | toSVG()로 Rect/Path/Line 벡터 출력 확인 | 📋 |
| 1-2 | 표(Table) 벡터 내보내기 | Table Group의 각 요소가 SVG로 정상 출력되는지 확인 | 📋 |
| 1-3 | opentype.js 텍스트 아웃라인 | 모든 텍스트를 SVG path로 자동 변환 (표 내부 텍스트 포함) | 📋 |
| 1-4 | Illustrator 호환 테스트 | SVG/PDF를 Illustrator에서 열어 벡터 편집 가능 확인 | 📋 |
| 1-5 | CMYK PDF 내보내기 | ICC FOGRA39 기반 CMYK 색상 정확도 확인 | 📋 |

### Phase 2: 줄자(Ruler) 기능
**목표**: Adobe Illustrator와 동일한 정밀 줄자 + 가이드라인
**예상 기간**: 1일

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 2-1 | 상단/좌측 눈금자 렌더링 | mm 단위, zoom/pan 동기화 | 📋 |
| 2-2 | 가이드라인 드래그 | 눈금자에서 드래그하여 가이드라인 생성 | 📋 |
| 2-3 | 스냅 기능 | 객체가 가이드라인에 스냅 | 📋 |
| 2-4 | 정밀도 검증 | mm 단위 오차 ±0.1mm 이내 확인 | 📋 |
| 2-5 | 그라데이션(Gradient) 기능 | 선형/원형 그라데이션, CMYK 호환, Illustrator 호환 내보내기 | 📋 |

### Phase 3: AI 시스템 전면 교체
**목표**: OpenAI 제거 → Recraft V4 Vector API 통합, 벡터 SVG 직접 생성
**예상 기간**: 2~3일

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 3-1 | OpenAI API 전체 제거 | generate-image, generate-design, review-design, generate-copy, extract-colors 제거 | 📋 |
| 3-2 | Recraft V4 Vector API 연동 | API 키 설정, SDK 설치, 테스트 | 📋 |
| 3-3 | AI 벡터 디자인 생성 UI | 로고 벡터화, 일러스트, 배경패턴, 아이콘 SVG 생성 | 📋 |
| 3-4 | 브랜드 컬러 팔레트 연동 | 사용자 브랜드 색상을 Recraft 컬러팔레트로 전달 | 📋 |
| 3-5 | 생성된 SVG → Fabric.js 로드 | loadSVGFromString()으로 캔버스에 벡터 객체로 배치 | 📋 |
| 3-6 | 우측 패널 AI 메뉴 재구성 | AI 벡터 디자인 + AI 추천 템플릿 2개 메뉴로 단순화 | 📋 |

### Phase 4: Panel Map 시스템
**목표**: 칼선 전개도의 각 면(앞면, 뒷면, 옆면, 날개, 플랩 등)을 자동 인식하여 면별 색상/이미지/템플릿 적용
**예상 기간**: 3~5일 (대표자 도메인 지식 필수)

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 4-1 | Panel Data Structure 구현 | id, label, type, path, holes, bounds, designable 속성 | 📋 |
| 4-2 | 1순위 박스 5종 면 경로 수학 공식 | 맞뚜껑(A식), 조립박스(B식), 택배박스(FEFCO-0201), 슬리브, 싸바리 | 📋 |
| 4-3 | SVG clipPath 면별 채우기 | 면 path → Fabric.js clipPath 변환, 색상/이미지/패턴 클리핑 | 📋 |
| 4-4 | 면 선택 UI | 캔버스에서 면 클릭 시 해당 면 하이라이트 + 속성 패널 | 📋 |
| 4-5 | 면별 디자인 요소 배치 | 면 상대좌표로 요소 배치, 치수 변경 시 자동 재배치 | 📋 |
...
| 5-5 | AI 실시간 벡터 생성 | 사용자가 "고급스러운 화장품 느낌" 입력 → Recraft V4로 SVG 생성 → 면에 자동 배치 | 📋 |
...
| 6-3 | Bleed Guide (재단 여백) | 5mm/10mm 블리드 가이드 표시 | 📋 대기 |
| 6-4 | 별색 Separation PDF | 별색 분판 PDF 내보내기 | 📋 장기 과제 |
| 6-5 | 2순위 박스 5종 Panel Map | 디스플레이, 파우치, 필로우, 육각형, 서랍형 | 📋 대기 |
| 6-6 | 3D 목업 미리보기 | 칼선 전개도 → 3D 접힌 상태 미리보기 | 📋 장기 과제 |
