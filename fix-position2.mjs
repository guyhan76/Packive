...
// Find the exact hr line before Group button
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('<hr className="w-10 border-gray-200"') && 
      i + 2 < lines.length && lines[i + 2].includes('const c = fcRef.current') &&
      lines[i + 4] && lines[i + 4].includes('activeselection')) {
    ...
    // Replace the hr line with Position block
    lines.splice(i, 1, ...alignBlock);
    done++;
    console.log('Inserted Position buttons at line ' + (i + 1));
    break;
  }
}
