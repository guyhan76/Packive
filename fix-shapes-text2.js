...
// Step 0: Remove duplicate state lines if any
let removed=0;
for(let i=lines.length-1;i>=0;i--){
  if(lines[i]&&lines[i].includes("showShapePanel")&&lines[i].includes("useState")){
    lines.splice(i,1); removed++; console.log("Removed duplicate showShapePanel at "+(i+1));
  }
  if(lines[i]&&lines[i].includes("showTextPanel")&&lines[i].includes("useState")){
    lines.splice(i,1); removed++; console.log("Removed duplicate showTextPanel at "+(i+1));
  }
}
console.log("Step 0: Removed",removed,"duplicate lines");
...
// Step 2: Find toolbar - lines 1602-1615 (0-indexed 1601-1614)
let toolStart=-1, toolEnd=-1;
for(let i=0;i<lines.length;i++){
  if(lines[i]&&lines[i].trim()==="{[" || (lines[i]&&lines[i].trim().startsWith("{[")&&lines[i+1]&&lines[i+1].includes("Select (V)"))){
    toolStart=i;
    console.log("Tool array start at line "+(i+1)+": "+lines[i].substring(0,80));
    break;
  }
}
if(toolStart===-1){
  for(let i=0;i<lines.length;i++){
    if(lines[i]&&lines[i].includes("icon:")&&lines[i].includes("Select (V)")){
      toolStart=i-1;
      console.log("Alt tool start at line "+(toolStart+1));
      break;
    }
  }
}
if(toolStart>-1){
  for(let i=toolStart;i<toolStart+25;i++){
    if(lines[i]&&lines[i].includes("].map((t")){
      // Find closing ))}
      for(let j=i;j<i+5;j++){
        if(lines[j]&&lines[j].trim().startsWith(")}")){
          toolEnd=j;
          console.log("Tool array end at line "+(j+1)+": "+lines[j].substring(0,80));
          break;
        }
      }
      break;
...
'',
'        {/* Text with font/size/weight/align popup */}',
...
'                {([["Thin","100"],["Light","300"],["Normal","400"],["Medium","500"],["Semi","600"],["Bold","700"],["ExBold","800"],["Black","900"]] as [string,string][]).map(([label,w])=>(',
...
'                <button onClick={()=>{const cv=fcRef.current;const obj=cv?.getActiveObject();if(obj&&obj.type==="i-text"){(obj as any).set("textAlign","left");cv?.renderAll();}}} className="flex-1 py-1.5 text-[11px] rounded text-gray-400 hover:bg-white/10 hover:text-white transition-all" title="Left">\u2261\uFE0E</button>',
'                <button onClick={()=>{const cv=fcRef.current;const obj=cv?.getActiveObject();if(obj&&obj.type==="i-text"){(obj as any).set("textAlign","center");cv?.renderAll();}}} className="flex-1 py-1.5 text-[11px] rounded text-gray-400 hover:bg-white/10 hover:text-white transition-all" title="Center">\u2630</button>',
'                <button onClick={()=>{const cv=fcRef.current;const obj=cv?.getActiveObject();if(obj&&obj.type==="i-text"){(obj as any).set("textAlign","right");cv?.renderAll();}}} className="flex-1 py-1.5 text-[11px] rounded text-gray-400 hover:bg-white/10 hover:text-white transition-all" title="Right">\u2630\uFE0E</button>',
'              </div>',
'              {/* Style */}',
...
'                      className="h-11 flex flex-col items-center justify-center rounded-lg text-lg text-gray-300 hover:text-white hover:bg-white/10 border border-white/5 hover:border-blue-500/30 transition-all">',
'                      <span>{s.icon}</span><span className="text-[7px] text-gray-500 mt-0.5">{s.tip}</span></button>',
'                  ))}',
'                </div>',
'              </div>',
'              <div>',
'                <div className="text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Circles</div>',
'                <div className="grid grid-cols-3 gap-1.5">',
...
// Replace lines toolStart to toolEnd (inclusive)
lines.splice(toolStart, toolEnd-toolStart+1, ...newToolbar);
console.log("Step 2: Toolbar replaced "+(toolEnd-toolStart+1)+" lines -> "+newToolbar.length+" lines");

// Step 3: Add new shape types to addShape
const codeNow=lines.join("\n");
if(!codeNow.includes("star6")){
  for(let i=0;i<lines.length;i++){
